...
# Calcular el valor promedio del IRA general
valor_promedio_ira = data_ira['ira'].mean()

# Función para obtener el estado y color según el valor ICA
def obtener_estado_color(ira):
    for rango, (estado, color) in rango_colores.items():
        if rango[0] <= ira <= rango[1]:
            return estado, color
    return "Desconocido", "black"

def obtener_diagnostico(ira):
    if 86 <= ira <= 100:
        return "Índice de ruido óptimo (30-40 dB). Excelente entorno acústico."
    elif 71 <= ira <= 85:
        return "Índice de ruido muy bueno (41-50 dB). Condiciones acústicas muy favorables."
    elif 51 <= ira <= 70:
        return "Índice de ruido bueno (50-55 dB). Condiciones adecuadas para la mayoría de las personas."
    elif 31 <= ira <= 50:
        return "Índice de ruido aceptable (55-65 dB). Puede haber molestias leves para personas sensibles."
    elif 6 <= ira <= 30.99:
        return "Índice de ruido regular (65-75 dB). Posibles molestias auditivas o estrés en ambientes prolongados."
    elif 0 <= ira <= 5:
        return "Índice de ruido malo (>75 dB). Niveles sonoros peligrosos. Se recomienda protección acústica."
    else:
        return "Valor de IRA fuera de rango. Verifica que esté entre 0 y 100."

# -------------------------------
# Categorización de IRA en "Estado"
# -------------------------------
def clasificar_ira(valor):
    if 86 <= valor <= 100:
        return "Óptimo"
    elif 71 <= valor <= 85:
        return "Muy Bueno"
        ...
        return "Valor de IRA fuera de rango"
    
# Aplicar clasificación directamente al DataFrame
data_ira["nivel_ruido"] = data_ira["ira"].apply(clasificar_ira)


# Generar el diagnóstico para todos los parques
diagnostico_general = obtener_diagnostico(valor_promedio_ira)

# Calcular el valor promedio de la calidad del aire
valor_promedio = data_ira['ira'].mean()

# Definir colores para los rangos de calidad del aire
rango_colores = {
    (86, 100): ("Óptimo", "darkgreen"),
    (71, 85): ("Muy Bueno", "lightgreen"),
    (51, 70): ("Bueno", "yellow"),
    (31, 50): ("Aceptable", "orange"),
    (6, 30.99): ("Regular", "red"),
    (0, 5): ("Malo", "blue"),
}
...
grafico_confort = px.bar(
    data_ira.groupby('nivel_ruido').size().reset_index(name='cantidad_parques'),
    x='nivel_ruido', y='cantidad_parques',
    title='Cantidad de parques por nivel de ruido ambiental',
...
grafico_confort.update_layout(
    ...
    title=dict(
        text="<b>Cantidad de parques vecinales por nivel de ruido ambiental</b>",  # 🔹 Texto en negrita
...
grafico_localidades = px.bar(
    data_ira.groupby(['localidad', 'nivel_ruido']).size().reset_index(name='cantidad_parques'),
    x='localidad', y='cantidad_parques',
    color='nivel_ruido',
    title='Cantidad de Parques por localidad y nivel de ruido ambiental',
    text='cantidad_parques',
    color_discrete_map={etiqueta: color for _, (etiqueta, color) in rango_colores.items()},
    barmode='stack'
)
...
grafico_localidades.update_layout(
    ...
    title=dict(
        text="<b>Cantidad de Parques por localidad y nivel de ruido ambiental</b>",  # 🔹 Negrita
        font=dict(size=13, family="Franklin Gothic Condensed, sans-serif", color="black"),  # 🔹 Fuente personalizada
...
fig_mapa = px.scatter_mapbox(
    data_ira, lat="lat", lon="lon", hover_name="parque",
    color="nivel_ruido", zoom=11,
    mapbox_style="carto-positron",
    color_discrete_map={etiqueta: color for _, (etiqueta, color) in rango_colores.items()},
    hover_data={"nivel_ruido": True, "localidad": True}  
)
...
localidades_promedio = data_ira.groupby('localidad')['ira'].mean().reset_index()
mejor_localidad = localidades_promedio.loc[localidades_promedio['ira'].idxmax()]
peor_localidad = localidades_promedio.loc[localidades_promedio['ira'].idxmin()]
...
app.layout = html.Div(
       ...
       # 🔹 DROPDOWN DE FILTROS POR INDICADORES
        html.Div(
            ...
            children=[
                ...
                dcc.Dropdown(
                    ...
                    value='IRA',  # Valor inicial cuando estás en el IRA
                    clearable=False,
                    style={
                        'fontSize': '0.7vw',
                        'fontFamily': 'Arial, sans-serif',
            ...
            children=[
                ...
                dbc.Modal(
                    [
                        ...
                        dbc.ModalBody(
                            html.Img(
                                src="/assets/ficha_infografica_ruido_ambiental.png",
                                style={
                                    "maxWidth": "100%",
                                    "maxHeight": "90vh",
                                    "width": "auto",
                        ...
                        dbc.ModalFooter([
                            html.A(
                                dbc.Button(
                                    "Ficha técnica indicador IRA",
                                    color="info",
                                    className="me-2"
                                ),
                                href="/assets/Indice_Ruido_Ambiental_IRA_Ficha_Indicador.pdf",
                                download="Indice_Ruido_Ambiental_IRA_Ficha_Indicador.pdf",
                                target="_blank"
                            ),
        ...
        html.Div(
            ...
            html.Div(
                ...
                children=[
                    dcc.Markdown("**Barra Indicador Ruido Ambiental (IRA General)**")
                ]
            ),
  ...
  # 📌 Titulo General
        html.Div(
            ...
            children=[
                dcc.Markdown("**RUIDO AMBIENTAL EN PARQUES VECINALES DE BOGOTÁ**")
            ]
        ),
     ...
     # 📌 DIAGNÓSTICO INDIVIDUAL DEL PARQUE
        ...
        # 📌 RANGOS DE RUIDO AMBIENTAL (TÍTULO)
                ...
                children=[
                    dcc.Markdown("**Rangos de Ruido Ambiental**")
                ]
            ),
                ...
                children=[
                        html.P([
                        html.Span("Óptimo (86-100) ", style={'color': 'black', 'fontWeight': 'normal'}), html.Span("| "), 
                        html.Span("Muy Bueno (71-85) ", style={'color': 'black', 'fontWeight': 'normal'}), html.Span("| "), 
                        html.Span("Bueno (51-70) ", style={'color': 'black', 'fontWeight': 'normal'}), html.Span("| "), 
                        html.Span("Aceptable (31-50) ", style={'color': 'black', 'fontWeight': 'normal'}), html.Span("| "), 
                        html.Span("Regular (6-30) ", style={'color': 'black', 'fontWeight': 'normal'}), html.Span("| "), 
                        html.Span("Malo (0-5)", style={'color': 'black', 'fontWeight': 'normal'})
                    ])
                ]
),
...
# 📌 TÍTULO DIAGNÓSTICO IRA GENERAL 
        html.Div(
            style={
                'fontSize': '0.7vw',
                'fontFamily': 'Franklin Gothic Condensed, sans-serif',  # Cambia la fuente
            ...
            children=[
                dcc.Markdown("**Diagnóstico IRA General**")
            ]
        ),
 ...
 # 📌 Titulo
    ...
    children=[
        dcc.Markdown("**Diagnóstico IRA parques vecinales por localidad**")
    ]
),
    ...
    # 📌 Balance de las localidades por calidad del aire en parques vecinales
        html.Div(
            ...
            children=[
                dcc.Markdown("**Balance de las localidades por ruido ambiental en parques vecinales**")
            ]
        ),
    ...
    # 📌 SELECCIONADOR LOCALIDADES
        ...
        style={
            ...
            'margin': '1',  # Elimina márgenes automáticos
                ...
                children=[
                html.P(f"En general, el ruido ambiental en los parques vecinales de Bogotá se ubica entre 60 y 65 dB diurnos, un nivel aceptable según la norma nacional, pero que supera los 55 dB recomendados por la OMS. Este ruido, que en algunos sectores excede los 65 dB, puede generar estrés y malestar, dificultando la comunicación, interrumpiendo la relajación, y provocando irritabilidad o fatiga en los usuarios, afectando negativamente su experiencia en estos espacios.", 
                    style={'textAlign': 'justify', 'color': 'black', 'fontSize': '0.7vw', 'fontWeight': 'normal'}),
                ]
            ),


# 📌 Ruido Ambiental por Localidad
html.Div(
    style={
        'backgroundColor': "#949191",
        'fontSize': '0.75vw',
...
children=[
    ...
    html.Span(f"{mejor_localidad['localidad']} - IRA promedio de {mejor_localidad['ira']:.2f}", 
    style={'fontSize': '0.75vw', 'color': 'darkgreen', 'fontWeight': 'bold'}),  
    html.Span("! "),  
    html.B("Peor Localidad: ", style={'color': 'black'}), 
    html.Span(f"{peor_localidad['localidad']} - IRA promedio de {peor_localidad['ira']:.2f}", 
...
def obtener_estado_color_ira(value):
    if 86 <= value <= 100:
        return 'Óptimo', 'darkgreen'
    elif 71 <= value <= 85:
        return 'Muy Bueno', 'lightgreen'
    elif 51 <= value <= 70:
        return 'Bueno', 'yellow'
    elif 31 <= value <= 50:
        return 'Aceptable', 'orange'
    elif 6 <= value <= 30.99:  # Aseguramos que los valores como 30.92 queden dentro de este rango
        return 'Regular', 'orangered'
    elif 0 <= value <= 5:
        return 'Malo', 'blue'
    else:
...
@app.callback(
    [
        Output('promedio-ira', 'children'),
        Output('nivel_ruido', 'children'),
        Output('nivel_ruido', 'style'),
        Output('promedio-decibelios', 'children'),
...
def actualizar_info(localidad):
    ...
    df_localidad = data_ira[data_ira['localidad'] == localidad]
    ...
    # Calcular métricas
    promedio_ira = df_localidad['ira'].mean(skipna=True)
    estado_ira, color_ira = obtener_estado_color(promedio_ira)  # 👈 ya no se sobrescribe
    promedio_decibelios = df_localidad['decibelios'].mean(skipna=True)
    ...
    return (
        html.Span([
            "Promedio IRA: ", 
            html.Span(f"{promedio_ira:.2f}", style={'color': color_ira, 'fontWeight': 'bold'})
        ]),

        html.Span([
            html.Span("Estado IRA: ", style={'color': 'black', 'fontWeight': 'bold'}),
            f"{estado_ira}"
        ]),
        ...
        html.Span([
            ...
            html.Span(f"{promedio_decibelios:.2f} db " , style={'color': 'black', 'fontWeight': 'bold'})
        ]),
...
if __name__ == '__main__':
    app.run_server(debug=True, host='0.0.0.0', port=8052)  # Puerto cambiado a 8051